...
def list_modules(module_path: Path, package_path: Path):
    ...
    module_path = os.path.dirname(
        importlib.import_module(package_name, str(package_path.stem)).__file__
    )
    modules = []
    for root, dirs, files in os.walk(module_path, topdown=True):
        for file in files:
            dirs[:] = [
                d for d in dirs if d not in ["build", "__pycache__"]
            ]  # exclude `build` directories, to avoid repeating .so files.
            if file.endswith(".so"):
                file = file.split(".")[0]
                module_name = os.path.splitext(
                    os.path.relpath(os.path.join(root, file), module_path)
                ...
                module = importlib.import_module(module_name, str(package_path.stem))
...
parser.add_argument(
    "--eval",
    help="strings to executed by Python before handling",
    default=None,
    type=str,
)
